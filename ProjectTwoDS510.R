...
plotFunc <- function(variable) {
  for (val in variable) {
    ...
    test<-list()
    rm(ttest)
    testt <- as.data.frame(matrix( nrow = 50))
    string <- as.vector(unlist(strsplit(val,"[+]")),mode="list")
    for (word in string){
      testt[paste(word)] <-heartTest[[word]]
    }
    within(testt, rm(V1))
    
    
    pred<-predict(glm.variable, testt)
    res.train <- (heartTest$AHD - pred)
    hist(res.train, prob=T, main = paste("Using",val))
...
dev.off()
# pdf(file = "ProjectTwoPlots2.pdf", onefile = TRUE)
plotFuncTwo(nine)
ttest
...
logit2 <- glm(formula = AHD ~ MaxHR+Oldpeak+Sex+ChestPain+ExAng+Slope+Ca+Thal, data=heartTrain, family="gaussian"(link="identity"))
summary(logit2)
...
new_data2 <-
  data.frame(
    MaxHR = heartTest$MaxHR,
    Oldpeak = heartTest$Oldpeak,
    Sex = heartTest$Sex,
    ChestPain = heartTest$ChestPain,
    ExAng = heartTest$ExAng,
    Slope = heartTest$Slope,
    Ca = heartTest$Ca,
    Thal = heartTest$Thal
  )
x =  rownames(MaxHR) = heartTest$MaxHR,
    "Oldpeak" = heartTest$Oldpeak,
    "Sex" = heartTest$Sex,
    "ChestPain" = heartTest$ChestPain,
    "ExAng" = heartTest$ExAng,
...
pred<-predict(logit2, new_data)
hist(pred)
res.train <- (heartTrain$AHD - pred)
hist(res.train)
#es.train <- heartTrain$pred)
hist(rest.test)
qqnorm(res.test)
qqline(res.test)

heartTest <- na.omit(heart.data[251:nrow(heart.data), ])
res.test <- (heart2$AHD - pred2)

pred2<-predict(logit2, new_data2)
hist(pred2)
heartTrain$pred <-pred
heartTrain$pred2 <-pred2
pred
table(pred,heartTrain$AHD)
min(pred)
...
test <- c(test,list(x=c(1:4)))
# after loop
test <- as.data.frame(test)
testt<-cbind(test,test)
...
library(stringr)
    f <- "2*(x1+x2-3*x3)"
    pattern <- "([A-z]+)"
    string <- as.vector(unlist(strsplit(two,"[+]")),mode="list")
    strsplit(nine,"[+]")
    testt<-list()
    new<-list()
    testt <- as.data.frame()
    ...
    for (word in string){
    testt[paste(word)] <-heartTest[[word]]
    # assign(paste(word), heartTest[[word]])
    # rbind(testt,paste(word))
    }

    test
    testt
    test <- c(list(Age=heartTest$Age))
    test <- c(list(Sex=heartTest$Sex))
