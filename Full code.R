...
heart.data <- read.csv("D:\\Users10\\Irina\\Desktop\\Heart4.csv", sep=",", header = TRUE)
...
cor(heartTrain, method="pearson") #with outliers, Pearson describes relation in terms of linearity
cor(heartTrain, method="kendall") #with outliers, Kendalln describes association between two quantities

#test for outliers in data
boxplot(heartTrain$Age, data=heartTrain, main="Boxplot of Age data")
boxplot(heartTrain$Sex, data=heartTrain, main="Boxplot of Sex data")
boxplot(heartTrain$ChestPain, data=heartTrain, main="Boxplot of ChestPain data")
boxplot(heartTrain$RestBP, data=heartTrain, main="Boxplot of RestBP data")
boxplot(heartTrain$Chol, data=heartTrain, main="Boxplot of Chol data")
boxplot(heartTrain$Fbs, data=heartTrain, main="Boxplot of Fbs MaxHR data")
boxplot(heartTrain$RestECG, data=heartTrain, main="Boxplot of RestECG data")
boxplot(heartTrain$MaxHR, data=heartTrain, main="Boxplot of MaxHR data")
boxplot(heartTrain$ExAng, data=heartTrain, main="Boxplot of ExAng data")
boxplot(heartTrain$Oldpeak, data=heartTrain, main="Boxplot of Oldpeak data")
boxplot(heartTrain$Slope, data=heartTrain, main="Boxplot of Slope data")
boxplot(heartTrain$Ca, data=heartTrain, main="Boxplot of Ca data")
boxplot(heartTrain$Thal, data=heartTrain, main="Boxplot of Thal data")
boxplot(heartTrain$AHD, data=heartTrain, main="Boxplot of AHD data")

#================================================================================== NOTES
# We need to remove variables that are very weakly correlated with AHD
# For categorical variables we don't remove outliers Sex ChestPain Thal ExAng Slope Ca
...
# We don't remove outliers categorical variables  (Sex ChestPain Thal ExAng Slope Ca)
Age = heartTrain$Age
Sex = heartTrain$Sex
ChestPain = heartTrain$ChestPain
RestECG = heartTrain$RestECG
...
hist(heartTrain$Age,prob=T,breaks=9, xlab="Age", main="Histogram of Age")
lines(density(heartTrain$Age),col="red")
qqnorm(heartTrain$Age, main="Normal Q-Q Plot of Age")
qqline(heartTrain$Age)

hist(heartTrain$MaxHR,prob=T,breaks=9, xlab="MaxHR", main="Histogram of MaxHR")
lines(density(heartTrain$MaxHR),col="red")
qqnorm(heartTrain$MaxHR, main="Normal Q-Q Plot of MaxHR")
qqline(heartTrain$MaxHR)

hist(heartTrain$Oldpeak,prob=T,breaks=9, xlab="Oldpeak", main="Histogram of Oldpeak")
lines(density(heartTrain$Oldpeak),col="red")
qqnorm(heartTrain$Oldpeak, main="Normal Q-Q Plot of Oldpeak")
qqline(heartTrain$Oldpeak)
plot(heart250$Oldpeak)

#reducing the number of observation in Oldpeak, remaining only those that are more than zero
heartTrain <- subset(heartTrain,Oldpeak > 0)
heartTrain <- na.omit(heartTrain)
...
cor(heartTrain, method="pearson")

#================================================================================
# These are the different combinations of the nine variables accepted variables
# We removed RestBP Chol Fbs because they were very weakly coorealated to AHD
#================================================================================
one <- c("Age",
         "Sex",
         "ChestPain",
...
plotPredictedHistogram <- function(variable) {
  for (val in variable) {
    ...
    confint(glm.variable, conf.level=0.95)
...
#First model
logit <- glm(formula = AHD ~ MaxHR+Oldpeak+Age+Sex+ChestPain+RestECG+ExAng+Slope+Ca+Thal, data=heartTrain, family="binomial"(link="logit"))
...
#Second model
logit2 <- glm(formula = AHD ~ Oldpeak+Age+Sex+ChestPain+RestECG+ExAng+Slope+Ca+Thal, data=heartTrain, family="gaussian"(link="identity"))
summary(logit2)
...
#choose Third model as our best model
logit3 <- glm(formula = AHD ~ Oldpeak+Sex+ChestPain+RestECG+ExAng+Slope+Ca+Thal, data=heartTrain, family="binomial"(link="logit"))
summary(logit3)
anova(object=logit3, test="Chisq")
BIC(logit3) #Higher numbers of Residual deviance indicates bad fit.  Deviance is a measure of goodness of fit of a model. 
pR2(logit3) # higher values indicating better model fit
res3 <- residuals(logit3)
hist(res3,prob=T,breaks=9, xlab="Residuals", main="Histogram of the residuals" )
lines(density(res3),col="red")
qqnorm(res3)
qqline(res3)
coef <- coefficients(logit3)
coef
confint(logit3,conf.level=0.95)

#Fourth model
logit4 <- glm(formula = AHD ~ Oldpeak+Sex+ChestPain+RestECG+ExAng+Ca+Thal, data=heartTrain, family="binomial"(link="logit"))
summary(logit4)
anova(object=logit4, test="Chisq")
BIC(logit4) #Higher numbers of Residual deviance indicates bad fit.  Deviance is a measure of goodness of fit of a model. 
pR2(logit4) # higher values indicating better model fit
res4 <- residuals(logit4)
hist(res4,prob=T,breaks=9, xlab="Residuals", main="Histogram of the residuals" )
lines(density(res4),col="red")
qqnorm(res4)
qqline(res4)

AHD_predicted <- predict(logit3,data=heart303, type="response") # Predicted values of AHD
AHD_predicted
